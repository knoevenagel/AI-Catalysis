...
def smooth_L1_loss(y_pred, y_test, sigma=1, dim=[1]):
    sigma_2 = sigma ** 2
    diff = tf.constant(y_pred - y_test,shape=(1,len(y_pred[0])),dtype=np.float32)
    abs_diff = (tf.abs(diff))
    #条件分岔
    smoothL1_sign = tf.stop_gradient(tf.cast(((tf.less(abs_diff,(1. / sigma_2)))),dtype=np.float32))
    print(smoothL1_sign)
    loss = tf.pow(diff, 2) * (sigma_2 / 2.) * smoothL1_sign + (abs_diff - (0.5 / sigma_2)) * (1. - smoothL1_sign)
    loss_box = tf.reduce_mean(tf.reduce_sum(loss, axis=dim))
    return loss_box
...
#       Reward = -critic_loss
    ...
    # Policy gradient loss computation. 第一项是MaE，第二项是sparse
